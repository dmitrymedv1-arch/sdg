...
def preprocess_text(text: str) -> tuple:
    """
    Preprocess text: clean, tokenize, and stem.
    Returns tuple of (original_lower, cleaned_text, stemmed_tokens_set, tokenized_sentences)
    """
    # Lowercase
    ...
    for word in words_only:
        if len(word) > 2:
            stemmed_tokens.add(word.lower())
    
    # Create token set for phrase matching
    token_set = set(all_tokens_lower)
    
    return text_lower, text_clean, stemmed_tokens, all_tokens_lower, token_set


# ============================================================================
# COMPLETE SDG KEYWORDS DATABASE (2000+ TERMS, HYBRID: EXACT PHRASES + STEMS)
# ============================================================================
...
def analyze_text(text: str) -> tuple:
    ...
    """
    # Preprocess text
    text_lower, text_clean, stemmed_tokens, all_tokens, token_set = preprocess_text(text)
    ...
    for sdg, keywords in SDG_KEYWORDS.items():
        ...
        for phrase, weight in keywords.get("exact_phrases", {}).items():
            ...
            if matches:
                tf = len(matches)
                # Weighted TF: weight * min(tf, 3) to cap at 3 occurrences
                weighted_score = weight * min(tf, 3)
                scores[sdg] += weighted_score
                matched_terms.append((sdg, phrase, "exact_phrase", weight, tf))
        ...
        for word, weight in keywords.get("single_words", {}).items():
            ...
            if has_whole_word(text_lower, word_lower):
                ...
                weighted_score = weight * min(tf, 3)
                scores[sdg] += weighted_score
                matched_terms.append((sdg, word, "exact_word", weight, tf))
        ...
        for stem, weight in keywords.get("stems", {}).items():
            ...
            if stem in stemmed_tokens:
                ...
                if matching_tokens:
                    ...
                    weighted_score = stem_weight * min(tf, 3)
                    scores[sdg] += weighted_score
                    matched_terms.append((sdg, f"{stem}*", "stem", stem_weight, tf))
...
<div style="text-align: center; margin-bottom: 2rem;">
    ...
    <div style="color: #666; font-size: 0.8rem; letter-spacing: 2px; margin-top: 0.5rem;">
        CHEMICAL & MATERIALS SCIENCE EDITION • 2000+ TERMS • WORD BOUNDARY MATCHING
    </div>
</div>
""", unsafe_allow_html=True)
...
with col_right:
    ...
    if st.session_state.analyzed and st.session_state.result_sdg:
        ...
        # Build HTML components in correct order
        sdg_icon = SDG_ICONS.get(primary_sdg, '🎯')
        sdg_name = SDG_NAMES.get(primary_sdg, 'Unknown')
        ...
        <div style="display: flex; gap: 1rem; justify-content: center; margin-top: 1rem; flex-wrap: wrap;">
            <span class="status-badge">🎯 SPECTRAL MATCH</span>
            <span class="status-badge">🔬 HYBRID MODE</span>
            <span class="status-badge">📊 TF WEIGHTED</span>
            <span class="status-badge">📚 2000+ TERMS</span>
        </div>
        """
        
        # Build complete card using list of HTML parts
        card_parts = []
        ...
        # Add multi-label badge if needed
        if analysis_result.get("multi_label"):
            card_parts.append("""
            <div style="text-align: center; margin-top: 0.5rem;">
                <span class="status-badge">🔗 MULTI-LABEL DETECTED</span>
            </div>
            """)
        ...
        if matched_terms:
            ...
            unique_terms = {}
            for sdg, term, match_type, weight, tf in matched_terms:
                if term not in unique_terms:
                    unique_terms[term] = {"type": match_type, "weight": weight, "tf": tf, "sdgs": [sdg]}
                else:
            ...
            for term, info in sorted_terms:
                ...
                <span class="{css_class}">
                    <span style="font-size: 0.6rem;">{icon}</span> {term}{tf_display}{sdg_display}
                </span>
                """, unsafe_allow_html=True)
...
<div style="margin-top: 2rem; padding: 1rem; text-align: center; border-top: 1px solid rgba(0,255,136,0.1);">
    <div style="font-size: 0.6rem; color: #444; letter-spacing: 1px;">
        SDG SPECTRAL ANALYZER v5.0 • HYBRID MODE (WORD BOUNDARY + STEM) • 2000+ TERMS • CHEMICAL & MATERIALS SCIENCE EDITION
    </div>
</div>
""", unsafe_allow_html=True)
...
with st.sidebar:
    ...
    <p><strong>SDG Spectral Analyzer v5.0</strong></p>
    ...
    <p>Database: 2000+ terms across all 17 SDGs</p>
